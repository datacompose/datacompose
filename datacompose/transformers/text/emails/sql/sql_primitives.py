...
class PrimitiveChooser:
    ...
    This class is responsible for discovering transformation functions from
    primitive classes and transpiling them to specific SQL dialects using SQLGlot.
    It provides a unified interface to generate SQL CREATE FUNCTION statements
    that can be executed across different database systems.
    
    Attributes:
        dialect (str): Target SQL dialect for transpilation (e.g., 'postgres', 
                      'mysql', 'snowflake', 'bigquery', 'sqlite')
    
    Example:
        >>> chooser = PrimitiveChooser('postgres')
        >>> sql_file = chooser.get_functions(Emails)
        >>> # Generates all email transformation functions for PostgreSQL
    """
    
    def __init__(self, dialect: str):
        """Initialize the PrimitiveChooser with a target SQL dialect.
        
        Args:
            dialect: The SQL dialect to transpile to. Supported values include
                    'postgres', 'mysql', 'snowflake', 'bigquery', 'sqlite', etc.
        """
    ...
    def get_functions(self, cls):
        ...
        Args:
            cls: A primitive class containing SQL function definitions as methods
                 (e.g., Emails, PhoneNumbers, Addresses). Each method should return
                 a SQL CREATE FUNCTION statement as a string.
        ...
        Example:
            ...
            >>> sql_functions = chooser.get_functions(Emails)
            >>> # sql_functions contains all email functions transpiled for Snowflake
            >>> with open('email_functions.sql', 'w') as f:
