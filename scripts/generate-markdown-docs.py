...
def extract_function_info(func_code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get decorator info
        has_compose = any(
            (isinstance(d, ast.Attribute) and d.attr == "compose")
            or (isinstance(d, ast.Name) and d.id == "compose")
...
def scan_transformers_directory(
...
def get_dialect_import(lib_name: str, dialect: str) -> str:
    ...
    lib_map = {"emails": "emails", "addresses": "addresses", "phone-numbers": "phones"}
...
def generate_markdown_file(
    lib_name: str, dialect: str, methods: List[Dict[str, Any]], output_dir: Path
):
    ...
    for file, file_methods in sorted(methods_by_file.items()):
        ...
        for method in file_methods:
            ...
            if method["has_compose"]:
                content.append(
                    "*This method is available via the `@compose()` decorator*"
                )
                content.append("")
            ...
            if method["args"]:
                args_example = ", ".join(
                    [arg.split(":")[0].split("=")[0] for arg in method["args"]]
                )
...
---"""
    ...
    content.append(
        lib_descriptions.get(
            lib_name, f"Transform and clean {lib_name.replace('-', ' ')} in your data."
        )
    )
    content.append("")
    ...
    content.append(
        f"from datacompose.transformers.text.{lib_import} import {lib_import}"
    )
    content.append("")
...
def main():
    ...
    while (
        not (project_root / "datacompose").exists()
