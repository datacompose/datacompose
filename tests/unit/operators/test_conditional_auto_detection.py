...
class TestConditionalAutoDetection:
    ...
    def test_conditional_in_pipeline(self, spark):
        ...
        # Verify processing
        assert (
            collected[0]["processed"] == "PROCESSED_HELLO"
        )  # was lowercase, made upper
    ...
    def test_all_conditional_patterns(self, spark):
        ...
        for prefix, func_name in patterns_to_test:
            ...
            assert func_name in ns._conditionals, (
                f"Function {func_name} with prefix {prefix} should be conditional"
            )
            assert func_name not in ns._primitives, (
                f"Function {func_name} should not be in primitives"
            )
