...
class TestAddCommandDefaultTarget:
    ...
    def test_add_auto_default_with_single_target(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            config = {
                "version": "1.0",
                "targets": {"snowflake": {"output": "./transformers/snowflake"}},
            }
            with open("datacompose.json", "w") as f:
                json.dump(config, f)
            ...
            with patch(
                "datacompose.cli.commands.add.validate_platform", return_value=True
            ):
                with patch("datacompose.cli.commands.add._run_add") as mock_run:
                    mock_run.return_value = 0
            ...
            with patch(
                ...
                with patch("datacompose.cli.commands.add._run_add") as mock_run:
                    ...
                    result = runner.invoke(
                        cli, ["add", "emails", "--target", "postgres"]
                    )
    ...
    def test_add_uses_config_output_path(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            config = {
                "version": "1.0",
                "default_target": "pyspark",
                "targets": {"pyspark": {"output": "./custom/build/path"}},
            }
            with open("datacompose.json", "w") as f:
                json.dump(config, f)
            ...
            with patch("datacompose.cli.commands.add._run_add") as mock_run:
                ...
                assert (
                    mock_run.call_args[0][2] is None
                )  # output not explicitly set in command
    ...
    def test_add_empty_targets_in_config(self, runner):
        ...
        with runner.isolated_filesystem():
            config = {"version": "1.0", "default_target": "pyspark", "targets": {}}
            with open("datacompose.json", "w") as f:
                json.dump(config, f)
    ...
    def test_add_uses_target_output_from_config(self, runner):
        ...
        with runner.isolated_filesystem():
            config = {
                "version": "1.0",
                "default_target": "pyspark",
                "targets": {"pyspark": {"output": "./my-custom-build"}},
            }
            with open("datacompose.json", "w") as f:
                json.dump(config, f)
            ...
            with patch(
                ...
                with patch(
                    ...
                    mock_instance.resolve_transformer.return_value = (
                        "emails",
                        Path("/path"),
                    )
    ...
    def test_add_explicit_output_overrides_config(self, runner):
        ...
        with runner.isolated_filesystem():
            config = {
                "version": "1.0",
                "default_target": "pyspark",
                "targets": {"pyspark": {"output": "./config-output"}},
            }
            with open("datacompose.json", "w") as f:
                json.dump(config, f)
            ...
            with patch("datacompose.cli.commands.add._run_add") as mock_run:
                ...
                result = runner.invoke(
                    cli, ["add", "emails", "--output", "./explicit-output"]
                )
    ...
    def test_add_fallback_output_when_not_in_config(self, runner):
        ...
        with runner.isolated_filesystem():
            config = {
                "version": "1.0",
                "default_target": "pyspark",
                "targets": {"pyspark": {}},  # No output specified
            }
            with open("datacompose.json", "w") as f:
                json.dump(config, f)
