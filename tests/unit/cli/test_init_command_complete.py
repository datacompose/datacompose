...
class TestInitCommand:
    ...
    def test_create_directory_structure_verbose(self):
        ...
        config = {
            "version": "1.0",
            "targets": {"pyspark": {"output": "./build/pyspark"}},
        }
        original_dir = os.getcwd()
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_setup_shell_completion_already_configured(self, mock_file, mock_exists):
        ...
        mock_file.return_value.read.return_value = (
            'eval "$(register-python-argcomplete datacompose)"'
        )
    ...
    @patch("pathlib.Path.exists", return_value=True)
    @patch("builtins.open")
    def test_setup_shell_completion_permission_denied_read(
        self, mock_open_func, mock_exists
    ):
    ...
    def test_prompt_completion_setup_failed(self, mock_input, mock_setup):
        ...
        with patch("builtins.print") as mock_print:
            ...
            # Should print manual instructions
            assert any(
                "Manual setup instructions" in str(call)
...
class TestPromptForTargets:
    ...
    def test_prompt_for_targets_custom_output(
        self, mock_input, mock_print, mock_get_key
    ):
    ...
    @patch("builtins.print")
    @patch("builtins.input", return_value="")
    def test_prompt_for_targets_multiple_selection(
        self, mock_input, mock_print, mock_get_key
    ):
...
class TestPromptForConfig:
    ...
    def test_prompt_for_config_success(self, mock_print, mock_targets):
        ...
        mock_targets.return_value = {"pyspark": {"output": "./transformers/pyspark"}}
