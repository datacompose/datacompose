...
class TestDatetimeExtraction:
    ...
    def test_extract_components(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["date_str", "expected_year", "expected_month", "expected_day"]
        )
        result_df = (
            df.withColumn("year", datetimes.extract_year(F.col("date_str")))
        ...
        for row in results:
            assert (
                row["year"] == row["expected_year"]
            ), f"Year extraction failed for '{row['date_str']}'"
            assert (
                row["month"] == row["expected_month"]
            ), f"Month extraction failed for '{row['date_str']}'"
            assert (
                row["day"] == row["expected_day"]
            ), f"Day extraction failed for '{row['date_str']}'"
    ...
    def test_natural_language_parsing(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["expression", "expected_interpretation"]
        )
...
class TestDatetimeParsing:
    ...
    def test_parse_incomplete_dates(self, create_session):
        ...
        df = create_session.createDataFrame(test_data, ["date_str", "expected"])
        result_df = df.withColumn("parsed", datetimes.parse_flexible(F.col("date_str")))
...
class TestDatetimeArithmetic:
    ...
    def test_date_differences(self, create_session, backend):
        ...
        df = create_session.createDataFrame(
            minutes_data, ["date1", "date2", "expected"]
        )
        result_df = df.withColumn(
        ...
        df = create_session.createDataFrame(
            seconds_data, ["date1", "date2", "expected"]
        )
        result_df = df.withColumn(
    ...
    def test_business_days_calculation(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["start_date", "end_date", "expected_days"]
        )
        result_df = df.withColumn(
            "business_days",
            datetimes.business_days_between(F.col("start_date"), F.col("end_date")),
        )
...
class TestDatetimeSpecialFunctions:
    ...
    def test_fiscal_periods(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["date", "fiscal_start_month", "expected_fy"]
        )
        result_df = df.withColumn(
            "fiscal_year",
            datetimes.fiscal_year(F.col("date"), F.col("fiscal_start_month")),
        )
    ...
    def test_quarter_operations(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["date", "expected_quarter", "expected_start", "expected_end"]
        )
        result_df = (
            df.withColumn("quarter", datetimes.extract_quarter(F.col("date")))
            .withColumn("quarter_start", datetimes.start_of_quarter(F.col("date")))
            .withColumn("quarter_end", datetimes.end_of_quarter(F.col("date")))
        )
    ...
    def test_age_calculation(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["birthdate", "reference", "expected_age"]
        )
        result_df = df.withColumn(
    ...
    def test_week_operations(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["date", "expected_week", "expected_day"]
        )
        result_df = df.withColumn(
            "week_of_year", datetimes.extract_week_of_year(F.col("date"))
        ).withColumn("day_of_week", datetimes.extract_day_of_week(F.col("date")))
...
class TestDatetimeEdgeCases:
    ...
    def test_extreme_dates(self, create_session):
        ...
        result_df = df.withColumn("is_valid", datetimes.is_valid_date(F.col("date")))
