...
class TestTimezoneDetection:
    ...
    def test_utc_variations(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["datetime_str", "expected_time", "expected_tz"]
        )
        result_df = df.withColumn(
            "normalized",
            datetimes.normalize_timezone(F.col("datetime_str"), F.lit("UTC")),
        )
...
class TestTimezoneConversions:
    ...
    def test_utc_to_local_conversions(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["utc_time", "target_tz", "expected"]
        )
        result_df = df.withColumn(
            "local_time",
            datetimes.normalize_timezone(F.col("utc_time"), F.col("target_tz")),
        )
    ...
    def test_local_to_utc_conversions(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["local_time", "source_tz", "expected_utc"]
        )
        result_df = df.withColumn(
    ...
    def test_cross_timezone_conversions(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["time", "from_tz", "to_tz", "expected"]
        )
...
class TestDaylightSavingTime:
    ...
    def test_spring_forward_transition(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["datetime", "timezone", "expected_tz", "is_dst"]
        )
    ...
    def test_no_dst_timezones(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["datetime", "timezone", "is_dst"]
        )
...
class TestTimezoneEdgeCases:
    ...
    def test_historical_timezone_changes(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["datetime", "timezone", "expected_dst"]
        )
    ...
    def test_naive_datetime_handling(self, create_session):
        ...
        result_df = df.withColumn(
            ...
            "with_est",
            datetimes.add_timezone(F.col("datetime_str"), F.lit("America/New_York")),
        )
    ...
    def test_remove_timezone_info(self, create_session):
        ...
        df = create_session.createDataFrame(
            test_data, ["datetime_str", "expected_naive"]
        )
        result_df = df.withColumn(
